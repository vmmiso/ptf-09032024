...
const characterMock = {
  ...
  urls: [
    {
      type: 'detail',
      url: 'http://marvel.com/characters/25/hulk?utm_campaign=apiRef&utm_source=d20131bc7707c02324ec158386bf2711',
    },
    {
      type: 'wiki',
      url: 'http://marvel.com/universe/Hulk_(Bruce_Banner)?utm_campaign=apiRef&utm_source=d20131bc7707c02324ec158386bf2711',
    },
    {
      type: 'comiclink',
      url: 'http://marvel.com/comics/characters/1009351/hulk?utm_campaign=apiRef&utm_source=d20131bc7707c02324ec158386bf2711',
    },
  ],
};
...
test('Renders Character card component', async () => {
  expect(await screen.findByText('Hulk')).not.toBeNull();
});

test('Checks fav icon click', async () => {
  const favIcon = await screen.getAllByRole('unfaved-icon')[0];
  expect(favIcon).not.toBeNull();
  fireEvent.click(favIcon);
  const unfavIcon = await screen.getAllByRole('faved-icon')[0];
  expect(unfavIcon).not.toBeNull();
  fireEvent.click(unfavIcon);
  expect(await screen.getAllByRole('unfaved-icon')[0]).not.toBeNull();
});
