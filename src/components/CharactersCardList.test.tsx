...
beforeEach(() => {
  render(
    <ThemeProvider theme={mainTheme}>
      <BrowserRouter>
        <FavsContextProvider>
          <CharactersCardList characters={charactersMock} />
        </FavsContextProvider>
      </BrowserRouter>
    </ThemeProvider>,
  );
...
test('Renders Characters card list component', async () => {
  expect(await screen.findAllByText('Hulk')).not.toBeNull();
});
...
test('Checks fav icon click', async () => {
  ...
  expect(await screen.getAllByRole('faved-icon')[0]).not.toBeNull();
});
